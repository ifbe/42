...
int unix_read(unsigned int fd, char *buf, int count)
{
	//buf = virt2phys(buf);
	return 0;
}
//1	sys_write	unsigned int fd	const char *buf	size_t count			
int unix_write(unsigned int fd, const char *buf, int count)
{
	return 0;
}
//2	sys_open	const char *filename	int flags	int mode			
int unix_open(const char *filename, int flags, int mode)
{
	//filename = virt2phys(filename);
	return 0;
}
//3	sys_close	unsigned int fd					
int unix_close(unsigned int fd)
{
	return 0;
}
...
void unix_syscall(u64 req, u64* arg)
{
	switch(req){
	case 0:unix_read( arg[0], (void*)arg[1], arg[2]);break;
	case 1:unix_write(arg[0], (void*)arg[1], arg[2]);break;
	case 2:unix_open((void*)arg[0], arg[1], arg[2]);break;
	case 3:unix_close(arg[0]);break;
	}
}
