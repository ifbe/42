...
{
	logtoall("@exception_spsel0_sync\n");
	for(;;);
}
void exception_spsel0_irq(void* regs)
{
	logtoall("@exception_spsel0_irq\n");
	for(;;);
}
void exception_spsel0_fiq(void* regs)
{
	logtoall("@exception_spsel0_fiq\n");
	for(;;);
}
void exception_spsel0_serr(void* regs)
{
	logtoall("@exception_spsel0_serr\n");
	for(;;);
}
...
void exception_spsel1_sync(void* regs)
{
	...
	logtoall("@exception_spsel1_sync: esr=%llx,far=%llx\n", esr, far);
...
{
	//logtoall("@exception_spsel1_irq: esr=%llx,far=%llx\n", esr, elr, spsr, far);
	gic4_isr(regs);
}
void exception_spsel1_fiq(void* regs)
{
	logtoall("@exception_spsel1_fiq\n");
	for(;;);
}
void exception_spsel1_serr(void* regs)
{
	logtoall("@exception_spsel1_serr\n");
	for(;;);
}
...
void exception_lowel64_sync(void* regs)
{
	...
	asm("mrs %0, esr_el1" : "=r"(esr));
	asm("mrs %0, far_el1" : "=r"(far));
	logtoall("@exception_lowel64_sync: esr=%llx,far=%llx\n", esr, far);
...
{
	logtoall("@exception_lowel32_sync\n");
	for(;;);
}
void exception_lowel32_irq(void* regs)
{
	logtoall("@exception_lowel32_irq\n");
	for(;;);
}
void exception_lowel32_fiq(void* regs)
{
	logtoall("@exception_lowel32_fiq\n");
	for(;;);
}
void exception_lowel32_serr(void* regs)
{
	logtoall("@exception_lowel32_serr\n");
	for(;;);
}
...
{
	logtoall("@initexception: vector@%p\n", vec);
