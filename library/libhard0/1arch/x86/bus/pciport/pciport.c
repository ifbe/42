...
void initpci_port()
{
	...
	for(fun=0;fun<8;fun++){
		...
		switch(type >> 16){
		case 0x0101:
			xx = device_alloc_fromtype(_ide_);
			ide_portinit(xx, addr);
			break;

		case 0x0106:
			if(0x01 == ((type>>8)&0xff)){
				xx = device_alloc_fromtype(_ahci_);
		...
		case 0x0108:
			...
			case 0x02:
			case 0x03:
				xx = device_alloc_fromtype(_nvme_);
		...
		case 0x0200:
			if(0x100e8086 == idid){
				xx = device_alloc_fromtype(_eth_);
		...
		case 0x0c03:
			...
			case 0x20:
				...
				break;
			case 0x30:
				xx = device_alloc_fromtype(_xhci_);
				xhci_portinit(xx, addr);
				break;
			case 0x40:
				xx = device_alloc_fromtype(_usb4_);
