...
{
	logtoall("@fuckgfwclient_write: %llx, %.4s, %d\n", self->pchip, &self->foottype, len);
...
{
	logtoall("@fuckgfwclient_detach: %.4s\n", &self->foottype);
	return 0;
}
int fuckgfwclient_attach(struct halfrel* self, struct halfrel* peer)
{
	logtoall("@fuckgfwclient_attach: %.4s\n", &self->foottype);
	return 0;
}
...
{
	logtoall("@fuckgfwserver_detach: %.4s\n", &self->foottype);
	return 0;
}
int fuckgfwserver_attach(struct halfrel* self, struct halfrel* peer)
{
	logtoall("@fuckgfwserver_attach: %.4s\n", &self->foottype);
	return 0;
}
