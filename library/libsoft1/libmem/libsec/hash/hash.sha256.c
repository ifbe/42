...
#define SWAP_BYTES
#ifdef SWAP_BYTES
#define BSWP(x,y)  _bswapw((u32 *)(x), (u32)(y))
#else
#define BSWP(p,n)
#endif
...
/* -------------------------------------------------------------------------- */
static void _bswapw(u32 *p, u32 i)
{
...
void * __cdecl _memcp (void *d, const void *s, u32 sz)
{
	void *rv = d;
...
/* -------------------------------------------------------------------------- */
static void _rtrf(u32 *b, u32 *p, u32 i, u32 j)
{
...
static void _hash(SHA256_CTX *ctx)
{
	...
	b[0] = ctx->hash[0]; b[1] = ctx->hash[1];
	b[2] = ctx->hash[2]; b[3] = ctx->hash[3];
	b[4] = ctx->hash[4]; b[5] = ctx->hash[5];
	b[6] = ctx->hash[6]; b[7] = ctx->hash[7];

	for (p = ctx->buf, j = 0; j < 64; j += 16)
	{
		_rtrf(b, p,  0, j), _rtrf(b, p,  1, j),
		_rtrf(b, p,  2, j), _rtrf(b, p,  3, j),
		_rtrf(b, p,  4, j), _rtrf(b, p,  5, j),
		_rtrf(b, p,  6, j), _rtrf(b, p,  7, j),
		_rtrf(b, p,  8, j), _rtrf(b, p,  9, j),
		_rtrf(b, p, 10, j), _rtrf(b, p, 11, j),
		_rtrf(b, p, 12, j), _rtrf(b, p, 13, j),
		_rtrf(b, p, 14, j), _rtrf(b, p, 15, j);
	}
...
/* -------------------------------------------------------------------------- */
void sha256_write(SHA256_CTX *ctx, u8 *dat, u32 sz)
{
	register u32 i = ctx->len[0] & 63, l, j;
...
/* -------------------------------------------------------------------------- */
void sha256_read(SHA256_CTX *ctx, u8 *buf)
{
	u32 i = (u32)(ctx->len[0] & 63), j = ((~i) & 3) << 3;
...
void sha256_start(SHA256_CTX ctx[1])
{
...
void sha256_stop(SHA256_CTX ctx[1])
{
}
void sha256sum(u8* dst, u8* src, int len)
{
	...
	{
		sha256_write(&ctx, src+j, 64);
	}
	if((len%64) > 0)sha256_write(&ctx, src+j, len%64);
	sha256_read(&ctx, dst);
