...
void sha256_create(SHA256_CTX ctx[1])
{
...
void sha256_delete(SHA256_CTX ctx[1])
{
}
void sha256sum(u8* dst, u8* src, int len)
{
        int j;
        SHA256_CTX context;
        sha256_create(&context);

        //
        for(j=0;j<=len-64;j+=64)
        {
                sha256_write(&context, src+j, 64);
        }
        if((len%64) > 0)sha256_write(&context, src+j, len%64);
        sha256_read(&context, dst);

        //
        sha256_delete(&context);
}
