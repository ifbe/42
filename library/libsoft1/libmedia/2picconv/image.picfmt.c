...
int picfmt_give(_obj* art,void* foot, _syn* stack,int sp, p64 arg, int cmd, void* buf, int len)
{
	...
	if(ispart){
		if(0 == per->srcbuf[0]){
			...
			per->srcbuf[0] = memory_alloc_align(per->srclen*2, 0);
		}
		if('y' == ispart){
	...
	if(0 == per->dstbuf[0]){
		...
		per->dstbuf[0] = memory_alloc_align(per->dstlen, 0);
	}
...
{
	//return artery_alloc();
	return memory_alloc_align(sizeof(struct item), sizeof(struct item));
}
