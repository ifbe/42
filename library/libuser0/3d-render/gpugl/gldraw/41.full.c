...
//logtoall("@update shader\n");
	...
	if(dst->shader_deq != src->shader_enq)
	{
		...
		buf1 = (void*)(src->fs);
		buf2 = (void*)(src->gs);
		//logtoall("shader:%llx,%llx,%llx\n", buf0, buf1, buf2);
		if((0 != buf0)&&(0 != buf1))
		{
			dst->shader = shaderprogram(buf0, buf1, buf2, 0, 0, 0);
			//logtoall("shader:(%llx,%llx,%llx)->%x\n", buf0, buf1, buf2, fd);
		}

		dst->shader_deq = src->shader_enq;
	}

//logtoall("@update vertex\n");
	//2: vertex
...
void update_ppll(struct gldst* dst, struct mysrc* src)
{
	...
	if(rbuf){
		logtoall("fd=%d,opaquecount=%d\n", dst->ppll_atomic, rbuf[0]);
	}
	else{
		logtoall("err@glMapBuffer:fd=%d,glGetError=%d\n", dst->ppll_atomic, glGetError());
	}
	...
	if(wbuf)wbuf[0] = 0;
	else logtoall("err@glMapBuffer:fd=%d,glGetError=%d\n", dst->ppll_atomic, glGetError());
	glUnmapBuffer(GL_ATOMIC_COUNTER_BUFFER);
	glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, 0);
	...
	if(data_rbuf){
		while((temp > 0) && (temp < 1024*768*4)){
			logtoall("data[%x]: prev=%d,temp=%d,color=%x,depth=%f\n", temp,
...
//logtoall("8888@opaque=%x, geom=%x, vbuf_h=%x, ibuf_h=%x\n", src->opaque, src->geometry, src->vbuf_h, src->ibuf_h);
...
{
	int j;
	//logtoall("fullwindow_render:%llx,%llx,%llx,%llx,%llx,%llx\n",cam,lit,solid,opaque,wnd,area);
...
int fullwindow_chooseandsend(_obj* wnd,void* foot, _syn* stack,int sp, p64 arg,int cmd, struct event* ev)
{
	...
	//logtoall("@gl41wnd0_write:(%p,%x)(%llx,%llx,%llx,%llx)\n", arg,cmd, ev->why, ev->what, ev->where, ev->when);
/*
	if(0x2b70 == ev->what){
		float color[4];
		float depth[4];
