...
//say("@freecam_read_byeye:%c\n",len);
	...
	if(stack){
		...
		if('v' == len)gbuffer_draw_gl41(act,slot, wrd,geom, wnd,area);
	}
...
//wnd.area -> cam.gl41, cam.slot -> world.geom
	...
	gl41data_before(wnd);
	gbuffer_draw_gl41(cam, 0, 0,(void*)&fs, wnd,area);
	gl41data_tmpcam(wnd);
	gl41data_after(wnd);
}
...
static int gbuffer_read(struct halfrel* self, struct halfrel* peer, struct halfrel** stack, int rsp, u8* buf, int len)
{
	...
	case _wnd_:{
		if('v' != len)break;
		gbuffer_read_bywnd(self, peer, stack, rsp, buf, len);break;
	}
	default:{
		gbuffer_read_bycam(self, peer, stack, rsp, buf, len);break;
	}
	}
	return 0;
}
