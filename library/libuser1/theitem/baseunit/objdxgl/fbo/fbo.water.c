...
static void water_read_bycam(struct halfrel* self, struct halfrel* peer, struct halfrel** stack, int rsp, void* buf, int len)
{
//wnd -> cam, cam -> world
...
//world -> water
	...
	if(stack){
		...
		if('?' == len){
			...
			water_forfbo_update(act,slot, win,geom, wrd,camg, fbo,rect);
			relationwrite(act,_fbo_, stack,rsp, buf,len);

			water_forwnd_update(act,slot, fbo,rect);
		}
	}
}
...
{
	water_read_bycam(self,peer, stack,rsp, buf,len);
}
