...
void gravity_acceltime_displace(struct style* geom, float dt)
{
	float la,lv,lx;
...
void gravity_acceltime_angular(struct style* geom, float dt)
{
	float a,invn,sbyn;
	vec4 v,ql,qr;
	struct fmotion* final = &geom->actual;
...
void gravity_acceltime(struct style** geom, int count, float dt)
{
	...
	for(j=0;j<count;j++){
		final = &geom[j]->actual;
		gravity_acceltime_displace(geom[j], dt);
		gravity_acceltime_angular(geom[j], dt);
