...
static void bplus_taking(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
}
static void bplus_giving(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
}
...
{
	if(0 == act)return;
	if(_copy_ == act->type)memory_free(act->listptr.buf0);
}
static void bplus_create(_obj* act)
{
	if(0 == act)return;
	if(_orig_ == act->type)act->listptr.buf0 = node;
	if(_copy_ == act->type)act->listptr.buf0 = memory_alloc_align(0x80*16, 0);
}
