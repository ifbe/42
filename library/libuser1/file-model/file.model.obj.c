...
static void obj3d_cpuctx_prep(struct privdata* own, char* albedo, char* matter, char* normal, void* arg)
{
	...
	own->objbuf = memory_alloc_align(0x200000, 0);
...
//logtoall("%s\n%s\n%s\n%s\n%s\n",albedo,matter,normal,vs,fs);
	...
	//shader
	src->vs = memory_alloc_align(0x10000, 0);
	loadglslfromfile(vs, 0, src->vs, 0x10000);
	src->fs = memory_alloc_align(0x10000, 0);
	...
	src->tex[0].data = memory_alloc_align(2048*2048*4, 0);
	...
	src->tex[1].data = memory_alloc_align(2048*2048*4, 0);
	...
	src->tex[2].data = memory_alloc_align(2048*2048*4, 0);
...
static void obj3d_attach(struct halfrel* self, struct halfrel* peer)
{
	...
	src->vtx[0].vbuf = memory_alloc_align(src->vtx[0].vbuf_len, 0);
...
static void obj3d_create(_obj* act, void* arg, int argc, u8** argv)
{
	...
	struct privdata* own = act->OWNBUF = memory_alloc_align(0x10000, 0);
	if(0 == own)return;
