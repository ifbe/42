...
void orthcam_fixcam(struct arena* win)
{
	float w,h,t;
	float x,y,z,norm;
...
void orthcam_rotatey(struct arena* win, float delta)
{
	float q[4];
	float va[4];
	float vb[4];
...
void orthcam_rotatex(struct arena* win, float delta)
{
...
void orthcam_rotatexy(struct arena* win, int dx, int dy)
{
	if(0 != dy)orthcam_rotatey(win, dy / 100.0);
	if(0 != dx)orthcam_rotatex(win, dx / 100.0);
}
...
void orthcam_zoom(struct arena* win, float delta)
{
	float va[4];
	float vb[4];
	...
	struct event* ev, int len)
{
	...
	else if(0x2b70 == ev->what)
	{
		id = (ev->why)>>48;
		switch(id){
			case 'f':orthcam_zoom(win, 100.0);break;
			case 'b':orthcam_zoom(win,-100.0);break;
			case 'r':return 1;
			default:return 0;
		}
	}
	else if(0x4070 == ev->what)
	{
			...
			(0 != win->input[1].z0))
		{
			...
			if((x0*x0+y0*y0) < (x1*x1+y1*y1))orthcam_zoom(win, 10.0);
			else orthcam_zoom(win, -10.0);
		}
		else
		{
			x0 = win->input[id].xn;
			...
			orthcam_rotatexy(win, x1-x0, y0-y1);
		}
	}
	...
	else if(joy_right == (ev->what & joy_mask))
	{
		...
		if((0 != x1) | (0 != y1))orthcam_rotatexy(win, x1/4096.0, y1/4096.0);
	}
...
static void orthcam_cread(
	struct actor* act, struct pinid* pin,
	struct arena* win, struct style* sty,
	u8* buf, int len)
{
}
static void orthcam_cwrite(
	struct actor* act, struct pinid* pin,
	struct arena* win, struct style* sty,
	u8* buf, int len)
{
}
static void orthcam_stop(
	struct actor* leaf, struct pinid* lf,
	struct arena* twig, struct style* tf,
	struct arena* root, struct style* rf)
{
}
static void orthcam_start(
	struct actor* leaf, struct pinid* lf,
	struct arena* twig, struct style* tf,
	struct arena* root, struct style* rf)
{
...
static void orthcam_create(void* addr)
{
    say("@orthcam_create\n");
}
