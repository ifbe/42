...
static void nor_read_byworld_bycam_bywnd(_obj* ent,void* slot, _syn* stack,int sp)
{
...
static void nor_taking(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
	if(0 == stack)return;
	...
	case _render_:
		break;
	default:
		nor_read_byworld_bycam_bywnd(ent,foot, stack,sp);
		break;
	}
}
static void nor_giving(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, u8* buf,int len)
{
	u8 tmp;
	logtoall("@norgate_write:%x\n",buf[0]);
