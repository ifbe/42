...
static void dancemat_read_byworld_bycam_bywnd(_obj* ent,void* slot, _syn* stack,int sp)
{
...
static void dancemat_taking(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
	...
	case _render_:
		break;
	default:
		dancemat_read_byworld_bycam_bywnd(ent,foot, stack,sp);
		break;
	}
}
...
static void dancemat_create(_obj* act, u8* arg, int argc, u8** argv)
{
	...
	act->VBUF = memory_alloc_align(0x1000, 0);

	nbuf = act->NBUF = memory_alloc_align(0x1000, 0);
	for(j=0;j<288;j++){
		nbuf[j] = (random_read() & 0xfff);
		//logtoall("%d\n", nbuf[j]);
	}

	ibuf = act->IBUF = memory_alloc_align(0x1000, 0);
	j = 0;
	for(y=0;y<120;y+=24){
		for(x=1;x<18;x+=4){
			k = y+x;
