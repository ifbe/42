...
static void fslist_delete(_obj* act)
{
	...
	if(priv->pathbuf){
		memory_free(priv->pathbuf);
		priv->pathbuf = 0;
	}
	if(priv->listbuf){
		memory_free(priv->listbuf);
		priv->listbuf = 0;
	}
}
static void fslist_create(_obj* act, void* arg, int argc, u8** argv)
{
	...
	struct str* path = priv->pathbuf = memory_alloc_align(0x10000, 0);
	path->len = copypath(path->buf, arg);

	struct str* list = priv->listbuf = memory_alloc_align(0x10000, 0);
	list->len = readfolder(path->buf,0, 0,0, list->buf,0x10000);
//logtoall("%d,%s\n", list->len, list->buf);
}
