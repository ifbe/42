...
static void mirror_taking(_obj* ent,void* slot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
	if(0 == stack)return;
...
static void mirror_giving(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
}
...
static void mirror_create(_obj* act, void* str)
{
	...
	struct mirrbuf* mirr = act->CTXBUF = memory_alloc_align(0x10000, 0);
	if(0 == mirr)return;
