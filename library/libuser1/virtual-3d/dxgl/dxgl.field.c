...
static void field_taking(_obj* ent,void* slot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
	if(0 == stack)return;
...
static void field_giving(_obj* ent,void* foot, _syn* stack,int sp, p64 arg,int key, void* buf,int len)
{
}
...
{
	float* buf = act->listptr.buf0 = memory_alloc_align(0x100000, 0);
}
