...
void drawunicode_error(struct actor* win, u32 rgb, int x, int y, u32 unicode)
{
	u8 ch;
...
void drawunicode(struct actor* win, u32 rgb, int xx, int yy, u32 unicode)
{
	u16 temp;
	u16* points;
	int x,y,offset;
...
void drawutf8(struct actor* win, u32 rgb, int xx, int yy, u8* ch, int len)
{
	u32 unicode;
	utf2unicode(ch, &unicode);
	drawunicode(win, rgb, xx, yy, unicode);
}
void drawutf8_fit(struct actor* win, u32 rgb, int x0, int y0, int x1, int y1, u8* ch, int len)
{
	u32 unicode;
	utf2unicode(ch, &unicode);
