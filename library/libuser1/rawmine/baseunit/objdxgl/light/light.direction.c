...
#define _fbo_ hex32('f','b','o',0)
	...
	dirlight_matrix(act,slot, win,geom, fbo,rect);
...
#define _fbo_ hex32('f','b','o',0)
	if(stack){
		...
		if('v' == len){
			dirlight_light(act,part, win,geom, wnd,area);
			dirlight_draw_vbo(act,part, win,geom, wnd,area);
		}
		if('?' == len){
			dirlight_shadowmap(act,part, win,geom, wrd,camg, stack,rsp, buf,len);
		}
	}
}
