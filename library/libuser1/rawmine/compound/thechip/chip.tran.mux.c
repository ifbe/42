...
static void mux_read(struct halfrel* self, struct halfrel* peer, struct halfrel** stack, int rsp, u8* buf, int len)
{
//wnd -> cam, cam -> world
...
//world -> mux
	...
	if(stack){
		...
		if('v' == len)mux_draw_vbo(act,slot, win,geom, wnd,area);
	}
}
