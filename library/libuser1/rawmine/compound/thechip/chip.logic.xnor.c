	...
	struct entity* ctx, struct style* area)
{
	...
	carveline_pmos(ctx, acolor, mcolor[0], tc,tr,tf,vt);
	for(j=0;j<3;j++){
		tc[j] = vc[j] +vr[j]/4 +vf[j]*3/4;
		tr[j] = vr[j]/4;
		tf[j] = vf[j]/4;
	}
	carveline_pmos(ctx, xcolor, mcolor[1], tc,tr,tf,vt);
	for(j=0;j<3;j++){
	...
	carveline_nmos(ctx, ycolor, mcolor[6], tc,tr,tf,vt);
	for(j=0;j<3;j++){
		tc[j] = vc[j] +vr[j]/4 -vf[j]*3/4;
		tr[j] = vr[j]/4;
		tf[j] = vf[j]/4;
	}
	carveline_nmos(ctx, bcolor, mcolor[7], tc,tr,tf,vt);
	...
	struct entity* ctx, struct style* area)
{
	...
	carveline_pmos(ctx, xcolor, tg_p, tc,tr,tf,vt);
	for(j=0;j<3;j++){
		tc[j] = vc[j] -vf[j]/4;
		tr[j] = vf[j]/4;
		tf[j] =-vr[j]/4;
	}
	carveline_nmos(ctx, bcolor, tg_n, tc,tr,tf,vt);
...
static void xnor_read(struct halfrel* self, struct halfrel* peer, struct halfrel** stack, int rsp, u8* buf, int len)
{
//wnd -> cam, cam -> world
...
//world -> xnor
	...
	if(stack){
		...
		if('v' == len)xnor_draw_vbo(act,slot, win,geom, wnd,area);
	}
}
static void xnor_write(struct halfrel* self, struct halfrel* peer, void* arg, int idx, u8* buf, int len)
{
	u8 tmp;
	struct entity* ent;
	//say("@xorgate_write:%x\n",buf[0]);
