...
static void jkff_draw(
	struct entity* act, struct style* pin,
	struct entity* win, struct style* sty)
{
	u64 fmt = win->fmt;
	if(fmt == _cli_)jkff_draw_cli(act, pin, win, sty);
	else if(fmt == _tui_)jkff_draw_tui(act, pin, win, sty);
	else if(fmt == _html_)jkff_draw_html(act, pin, win, sty);
	else if(fmt == _json_)jkff_draw_json(act, pin, win, sty);
	else jkff_draw_pixel(act, pin, win, sty);
}
...
//world -> jkff
	...
	if(stack){
		...
		if('v' == len)jkff_draw_vbo(act,slot, win,geom, wnd,area);
	}
}
