	...
	struct entity* ctx, struct style* area)
{
	...
	for(k=0;k<8192;k++){
		...
		for(j=0;j<3;j++){
			...
			v2[j] = vc[j] + vr[j]*x0 + vf[j]*y0 + vt[j]*rz;
			v1[j] = vc[j] + vr[j]*(x0+rx+fx) + vf[j]*(y0+ry+fy);
		}
...
static void grass_create(struct entity* act)
{
	...
	for(j=0;j<8192;j++){
		f[j*6+0] = (getrandom()&0xffff)/32768.0 - 1.0;
		f[j*6+1] = (getrandom()&0xffff)/32768.0 - 1.0;
		f[j*6+2] = 0.0;

		f[j*6+3] = (getrandom()&0xff)/65536.0;
		f[j*6+4] = (getrandom()&0xff)/65536.0;
		f[j*6+5] = (getrandom()&0xffff)/65536.0;
	}
}
